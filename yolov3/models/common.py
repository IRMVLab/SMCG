...
from yolov3.utils.datasets import exif_transpose, letterbox
...
from yolov3.utils.torch_utils import time_sync
...
class Detections:
    ...
    def detect_result(self):
        ...
        #                 box_.append(box[j].cpu().numpy())
        #             conf = conf.cpu().numpy()
        #             cls = cls.cpu().numpy()
        #             crops.append({'box': np.array(box_), 'conf': conf, 'cls': cls, 'label': label})
        ...
        for i, (im, pred) in enumerate(zip(self.imgs, self.pred)):
            ...
            if pred.shape[0]:
                for *box, conf, cls in reversed(pred):  # xyxy, confidence, class
                    ...
                    box_ = []
                    for j in range(len(box)):
                        box_.append(box[j].detach().cpu().numpy())
                    box_[0] /= 192
                    box_[1] /= 144
                    box_[2] /= 192
                    box_[3] /= 144
                    box_.append(conf.detach().cpu().numpy())
