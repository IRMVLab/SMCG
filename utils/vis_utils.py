...
for color in agent_colors:
    img = np.ascontiguousarray(
        np.flipud(imageio.imread("env_utils/agent_pictures/agent_{}.png".format(color)))
    )
...
def clip_map_birdseye_view(image, clip_size, pixel_pose):
    ...
    try:
        return_image[
            start_x : start_x + cliped_image.shape[0],
            start_y : start_y + cliped_image.shape[1],
        ] = cliped_image
    except:
        print(
            "image shape ",
...
def append_text_to_image(
    image: np.ndarray, text: str, font_size=0.5, font_line=cv2.LINE_AA
):
...
def observations_to_image(
    ...
    if "target_goal" in observation and len(observation["target_goal"]) > 0:
        ...
        if len(goal_rgb.shape) == 4:
            if info is not None:
                goal_rgb = goal_rgb * (1 - info["total_success"]).reshape(
                    -1, *[1] * len(goal_rgb.shape[1:])
                )
    ...
    if info is not None and "top_down_map" in info:
        if info["top_down_map"] is not None:
            top_down_height = (
                ...
                else info["top_down_map"]["map"].shape[0]
            )
            # print(frame.shape,"frameshape")
            top_down_map = info["top_down_map"]["map"]
            top_down_map[top_down_map == 7] = 1
            # print(top_down_map.shape,"tooopp")
            ...
            if history is not None and len(history) > 0:
                for i, h in enumerate(history):
                    top_down_map = draw_agent(
                        image=top_down_map,
                        agent_id=2,
                        agent_center_coord=h[0],
                        agent_rotation=h[1],
                        agent_radius_px=1,
                    )
            history.append((map_agent_pos, info["top_down_map"]["agent_angle"]))
        ...
        # print(top_down_map,top_down_map.shape)
        # top_down_map = color_top_down_map

        #  frame=None
        # print(frame.shape,top_down_map.shape,"dawqeqe")
        frame = (
            np.concatenate((frame, top_down_map), axis=1)
            if frame is not None
            else top_down_map
        )

    return frame, history, info["top_down_map"]["agent_angle"]
