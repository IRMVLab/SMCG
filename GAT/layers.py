...
class MultiLayerGAT(nn.Module):
    def __init__(self, nfeat, nhid, nclass, n_layers, dropout, alpha, nheads, device='cpu'):
        ...
        # 第1层
        self.layers.append(
            nn.ModuleList([GraphAttentionLayer(nfeat, nhid, dropout, alpha, concat=True).to(device)
        ...
        for _ in range(n_layers - 2):
            self.layers.append(
                nn.ModuleList([GraphAttentionLayer(nhid * nheads, nhid, dropout, alpha, concat=True).to(device)
                               for _ in range(nheads)])
            )
