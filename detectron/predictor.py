...
class VisualizationDemo:
    ...
    def run_on_image(self, image):
        ...
        predictions = self.predictor(image)
        objects = (
            predictions["instances"]._fields["pred_boxes"].tensor.cpu().detach().numpy()
        )
        scores = predictions["instances"]._fields["scores"].cpu().detach().numpy()
        classes = (
            predictions["instances"]._fields["pred_classes"].cpu().detach().numpy()
        )
        ...
        return objects, scores, classes, vis.get_image()[:, :, ::-1]
