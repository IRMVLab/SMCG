...
class HabitatSim(habitat_sim.Simulator, Simulator):
    ...
    def create_sim_config(
        ...
        sim_config.allow_sliding = self.habitat_config.HABITAT_SIM_V0.ALLOW_SLIDING
        agent_configs = []
        for id in range(self.num_agents):
            agent_config = habitat_sim.AgentConfiguration()
            overwrite_config(
                config_from=self._get_agent_config(id),
            ...
            for sensor in self.sensor_suite.sensors.values():
                if "equirect" in sensor.uuid:
                    sim_sensor_cfg = habitat_sim.EquirectangularSensorSpec()
                else:
                ...
                sensor = cast(HabitatSimSensor, sensor)
    ...
    def reset(self) -> Observations:
        ...
        return self._sensor_suite.get_observations(
            sim_obs
        )  # , dataset=self.habitat_config.SCENE.split("/")[-2])
    ...
    def set_agent_state(
        ...
        Args:
            ...
            reset_sensors: bool for if sensor changes (e.g.txt. tilt) should be
                reset).
