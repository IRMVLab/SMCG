...
def construct_envs(
    ...
    print(
        "Total Process %d = train %d + eval %d "
        % (total_num_processes, num_processes, num_val_processes)
    )
    ...
    for i in range(total_num_processes):
        ...
        task_config = add_panoramic_camera(
            task_config,
            has_target="search" in proc_config.ENV_NAME.lower()
            or getattr(proc_config, "TASK_TYPE", True),
        )

        task_config.SIMULATOR.HABITAT_SIM_V0.GPU_DEVICE_ID = config.SIMULATOR_GPU_ID
        task_config.SIMULATOR.HABITAT_SIM_V0.GPU_GPU = (
            habitat_sim.cuda_enabled and not fix_on_cpu
        )
            ...
            env_fn_args=tuple(
                tuple(
                    zip(
                        ...
                        [{"run_type": run_type}] * total_num_processes,
                    )
                )
            ),
        )
