...
class Env:
    ...
    def __init__(self, config: Config) -> None:
        ...
        self._swap_building_every = (
            config.ENVIRONMENT.ITERATOR_OPTIONS.MAX_SCENE_REPEAT_EPISODES
        )
        ...
        self._task = make_task(
            self._config.TASK.TYPE, config=self._config.TASK, sim=self._sim
        )
    ...
    def get_next_episode_search(self, episode_id, scene_id):
        ...
        while True:
            ...
            while True:
                ...
                random_angle = np.random.rand() * 2 * np.pi
                new_start_position = [
                    init_start_position[0] + random_dist * np.cos(random_angle),
            ...
            while True:
                ...
                if abs(goal_position[1] - init_start_position[1]) > 0.5:
                    continue
                geodesic_dist = self._sim.geodesic_distance(
                    self.start_position, goal_position
                )
                valid_dist = (geodesic_dist < self.MAX_DIST) and (
                    geodesic_dist > self.MIN_DIST
                )
                if self._sim.is_navigable(goal_position) and valid_dist:
                    goal = NavigationGoal(**{"position": goal_position})
                    goals.append(goal)

                if len(goals) >= self._num_goals or (
                    num_try > 1000 and len(goals) >= 1
                ):
                    found = True
                    break
                num_try += 1
    ...
    def reset(self) -> Observations:
        ...
        if self._current_scene_iter >= self._swap_building_every:
            ...
            print(
                "swapping building %s, every episode will be sampled in : %f, %f"
                % (scene_name, self.MIN_DIST, self.MAX_DIST)
            )

        while True:
            self._current_episode, found_episode = self.get_next_episode_search(
                self._current_scene_episode_idx, self._config.SIMULATOR.SCENE
            )
            if not found_episode:
                self._episode_iterator[scene_name] = self._current_scene_episode_idx + 1
                self._current_scene_idx = (self._current_scene_idx + 1) % len(
                    self._scenes
                )
                scene_name = self._scenes[self._current_scene_idx]
                if scene_name not in self._episode_iterator.keys():
        ...
        agent_dict = {
            ...
            "START_ROTATION": quat_to_coeffs(
                q.from_float_array(self._current_episode.start_rotation)
            ).tolist(),
