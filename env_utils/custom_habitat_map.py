...
def get_topdown_map(
    ...
    if map_resolution[0] == 1250:
        ...
        map_name = "explore_map/%s_%.2f_res%d.png" % (
            scene_name,
            start_height,
            map_resolution[0],
        )
    if os.path.exists(map_name) and not draw_new_map and map_resolution[0] == 1250:
        ...
        for _ in range(num_samples):
            ...
            if np.abs(point[1] - start_height) > height_th:
                continue
            g_x, g_y = to_grid(
        ...
        if not loose_check:
            ...
            while True:
                ...
                del queue[0]
                if pix_x < range_x[0] or pix_x > range_x[1]:
                    continue
                if pix_y < range_y[0] or pix_y > range_y[1]:
                    continue
                if distance_map[pix_x, pix_y] != -1:
                    continue
                    ...
                    queue.extend(
                        list(
                            np.concatenate(
                                [
                                    neighbor,
                                    np.ones([len(neighbor), 1], dtype=np.int32)
                                    * (distance + 1),
                                ],
                                1,
                            )
...
class TopDownGraphMap(Measure):
    ...
    def __init__(self, *args: Any, sim: Simulator, config: Config, **kwargs: Any):
        maps.TOP_DOWN_MAP_COLORS[COMMON_NODE] = [254, 129, 125]  # [119,91,138]
        maps.TOP_DOWN_MAP_COLORS[COMMON_EDGE] = [129, 184, 223]  # [189,164,204]
        ...
        self.milli_delta = 60
        self.delta_angs = [
            (2 * np.pi * i / self.milli_delta) for i in range(self.milli_delta)
        ]
    ...
    def _draw_boundary(self, position, point_type):
        ...
        self._top_down_map[
            t_x - padd : t_x + padd + 1, t_y - padd : t_y + padd + 1
        ] = original
    ...
    def _draw_shortest_path(self, episode: Episode, agent_position: AgentState):
        if self._config.DRAW_SHORTEST_PATH:
            ...
            # print(self._shortest_path_points)
    ...
    def reset_metric(self, *args: Any, episode, **kwargs: Any):
        ...
        #     save_path = os.path.join(save_directory, "traj{}.mat".format(self.i))
        #     scipy.io.savemat(save_path, mdict={"x": traj[:, 0], "y": traj[:, 1]})
        #     # scipy.io.savemat('traj{}.mat'.format(self.i), mdict={'x': traj[:, 0], 'y': traj[:, 1]})
        self.traj = []
        self.traj.append((a_x, a_y))
        self.i += 1
    ...
    def update_metric(self, episode, action, *args: Any, **kwargs: Any):
        ...
        clipped_fog_of_war_map = None
        if self._config.FOG_OF_WAR.DRAW:
            clipped_fog_of_war_map = self._clip_map(
    ...
    def update_map(self, agent_position, agent_rotation=None):
        ...
        points = [self._previous_xy_location[::-1], (a_x, a_y)]
        # print(points, self._shortest_path_points)
        if self._previous_xy_location[::-1] != (a_x, a_y):
            self.traj.append((a_x, a_y))
    ...
    def update_graph(
        self, node_list, affinity, graph_mask, curr_info={}, flags=None, goal_id=None
    ):
...
node = [
    np.array([-5.2530847, 0.17282087, 3.2406325]),
    np.array([-4.728492, 0.17282087, 2.1110723]),
    np.array([-3.4607134, 0.17282087, 0.24437541]),
    np.array([-2.9403458, 0.17282087, -3.171939]),
    np.array([-0.58548284, 0.17282087, -3.8160808]),
    np.array([0.1626797, 0.17282087, -2.5970502]),
    np.array([-1.8089252, 0.17282087, -3.8833482]),
    np.array([-2.8343298, 0.17282087, -4.9498324]),
]
