...
class SearchEnv(RLEnv):
    ...
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        if self.record or self.render_map:
            task_config.TASK.TOP_DOWN_GRAPH_MAP = (
                config.TASK_CONFIG.TASK.TOP_DOWN_MAP.clone()
            )
            ...
            if getattr(config, "map_more", False):
                ...
                task_config.TASK.TOP_DOWN_GRAPH_MAP.NUM_TOPDOWN_MAP_SAMPLE_POINTS = (
                    10000
                )
            task_config.TASK.TOP_DOWN_GRAPH_MAP.DRAW_CURR_LOCATION = getattr(
                config, "GRAPH_LOCATION", "point"
            )
        ...
        if "TOP_DOWN_MAP" in config.TASK_CONFIG.TASK.MEASUREMENTS:
            task_config.TASK.MEASUREMENTS = [
                k for k in task_config.TASK.MEASUREMENTS if "TOP_DOWN_MAP" != k
            ]
        ...
        if self.noise:
            moves = ["NOISY_FORWARD", "NOISY_LEFT", "NOISY_RIGHT"]
        else:
        ...
        print(
            self.habitat_env._task.sensor_suite.observation_spaces.spaces, "jaskdjkasdj"
        )
        obs_dict = {
            "panoramic_rgb": Box(
                low=0, high=255, shape=(144, 192, 3), dtype=np.float32
            ),  # self.habitat_env._task.sensor_suite.observation_spaces.spaces['panoramic_rgb'],
            "panoramic_depth": Box(
                low=0, high=1.0, shape=(144, 192, 1), dtype=np.float32
            ),  # self.habitat_env._task.sensor_suite.observation_spaces.spaces['panoramic_depth'],
            "target_goal": self.habitat_env._task.sensor_suite.observation_spaces.spaces[
                "target_goal"
            ],
            "step": Box(low=np.array(0), high=np.array(500), dtype=np.float32),
            "prev_act": Box(
                low=np.array(-1), high=np.array(self.action_space.n), dtype=np.int32
            ),
            "gt_action": Box(
                low=np.array(-1), high=np.array(self.action_space.n), dtype=np.int32
            ),
        ...
        self.mapper = (
            self.habitat_env.task.measurements.measures["top_down_map"]
            if (self.record or self.render_map)
            else None
        )
        ...
        if config.DIFFICULTY == "easy":
            ...
            self.habitat_env.MIN_DIST, self.habitat_env.MAX_DIST = 1.5, 10.0  # 1.5 10
        else:
            raise NotImplementedError
        print(
            "[SearchEnv] Current difficulty %s, MIN_DIST %f, MAX_DIST %f - # goals %d"
            % (
                config.DIFFICULTY,
                self.habitat_env.MIN_DIST,
                self.habitat_env.MAX_DIST,
                self.habitat_env._num_goals,
            )
        )
    ...
    def build_path_follower(self, each_goal=False):
        self.follower = ShortestPathFollower(
            self._env.sim, self.success_distance, False
        )
    ...
    def set_random_goals(self):
        ...
        while True:
            ...
            far = self.habitat_env._sim.geodesic_distance(pose, goal_pose) > 7.0
    ...
    def get_dist(self, goal_position):
        return self.habitat_env._sim.geodesic_distance(
    ...
    def recording_now(self):
        return (
            self.record
            and self.habitat_env._total_episode_id % self.record_interval == 0
        )
    ...
    def curr_goal(self):
        ...
        return self.current_episode.goals[
            min(self.curr_goal_idx, len(self.current_episode.goals) - 1)
        ]
    ...
    def process_obs(self, obs):
        ...
        obs["target_goal"] = (
            obs["target_goal"][0] if self.num_goals == 1 else obs["target_goal"]
        )
        ...
        if self.return_have_been:
            if len(self.positions) < 10:
                ...
                dists = np.linalg.norm(
                    ...
                    - np.expand_dims(np.array(self.current_position), 0),
                    axis=1,
                )
        ...
        if self.return_target_dist_score:
            target_dist_score = np.maximum(1 - np.array(obs["distance"]) / 2.0, 0.0)
            obs["target_dist_score"] = (
                np.array(target_dist_score).astype(np.float32).reshape(1)
            )
        return obs
    ...
    def step(self, action):
        ...
        """if self._previous_measure<1.0:
            action=0"""
        self._previous_action = action
        # print("apply:",self.action_dict[action])
    ...
    def save_video(self, imgs):
        ...
        # imgs = [cv2.resize(img2, dsize=(resize_h, resize_w)) for img2 in imgs]
    ...
    def get_done(self, observations):
        ...
        if self.stuck > 80 or diff_floor or self.time_t > 400:
            done = True
    ...
    def log_info(self, log_type="str", info=None):
        self.has_log_info = {"type": log_type, "info": info}

    def render(self, mode="rgb", history=None):
        if history is None:
            history = []

        info = self.get_info(None) if self.info is None else self.info
        img, history, angle = observations_to_image(
            self.obs, info, mode="panoramic", history=history
        )

        # print('curr_node',curr_info['curr_node'].cpu().numpy(),self.time_t)
        # img2 = append_text_to_image(img2, txt)

        if mode == "rgb" or mode == "rgb_array":
            # return img2

            return cv2.resize(img, dsize=(2000, 1250)), history, angle
        elif mode == "human":
        ...
        return super().render(mode), history

    def get_dists(self, pose, other_poses):
        dists = np.linalg.norm(
            np.array(other_poses).reshape(len(other_poses), 3)
            - np.array(pose).reshape(1, 3),
            axis=1,
        )
        return dists
    ...
    def step(self, action):
        ...
        if "STOP" in self.action_space.spaces and action == 0:
            ...
            if dist <= self.success_distance:
                ...
                all_done = self.habitat_env.task.measurements.measures[
                    "goal_index"
                ].increase_goal_index()
                state = self.habitat_env.sim.get_agent_state()
                obs = self.habitat_env._sim.get_observations_at(
                ...
                obs, reward, done, self.info = super(SearchEnv, self).step(
...
if __name__ == "__main__":
    ...
    config.TASK_CONFIG.DATASET.CONTENT_SCENES = ["Nicut"]  # environment
    ...
    while True:
        ...
        if key == ord("s"):
            ...
            action = 1
            cv2.imwrite(
                "OBS{}.png".format(save_node), obs["panoramic_rgb"][:, :, [2, 1, 0]]
            )
            save_node += 1
        elif key == ord("q"):
            break
    ...
    print(
        "Reset : ",
        [(key, np.array(reset_list).mean()) for key, reset_list in reset_time.items()],
    )
