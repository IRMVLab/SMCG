...
class PanoramicRGBSensor(Sensor):
    ...
    def get_observation(self, observations, *args: Any, **kwargs: Any):
        if isinstance(observations["rgb_0"][:, :, :3], torch.Tensor):
            rgb_list = [
                observations["rgb_%d" % (i)][:, :, :3] for i in range(self.num_camera)
            ]
...
class PanoramicDepthSensor(DepthSensor):
    def __init__(self, sim, config, **kwargs: Any):
        ...
        if config.NORMALIZE_DEPTH:
            ...
            self.depth_range = [config.MIN_DEPTH, config.MAX_DEPTH]
        self.min_depth_value = config.MIN_DEPTH
        self.max_depth_value = config.MAX_DEPTH
        self.num_camera = config.NUM_CAMERA
        super().__init__(config=config)
    ...
    def get_observation(self, observations, *args: Any, **kwargs: Any):
        ...
        if isinstance(observations["rgb_0"][:, :, :3], torch.Tensor):
            ...
            depth_array = torch.clamp(
                depth_array, min=self.min_depth_value, max=self.max_depth_value
            )
            # depth_array = depth_array.unsqueeze(2)
        else:
            depth_array = np.concatenate(depth_list, 1)
            depth_array = np.clip(
                depth_array, self.min_depth_value, self.max_depth_value
            )
...
class GoalIndex(Measure):
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any, **kwargs: Any):
        self._metric = {"curr_goal_index": self.goal_index, "num_goals": self.num_goals}

    def increase_goal_index(self):
        self.goal_index += 1
        self._metric = {
            "curr_goal_index": min(self.goal_index, self.num_goals - 1),
...
class DistanceToGoal(Measure):
    ...
    def update_metric(
        self, episode: Episode, task: EmbodiedTask, *args: Any, **kwargs: Any
    ):
        current_position = self._sim.get_agent_state().position

        if GoalIndex.cls_uuid in task.measurements.measures:
            self.goal_idx = task.measurements.measures[GoalIndex.cls_uuid].get_metric()[
                "curr_goal_index"
            ]
        else:
            self.goal_idx = 0
...
class SPL(Measure):
    ...
    def update_metric(self, episode, task: EmbodiedTask, *args: Any, **kwargs: Any):
        ...
        self._metric = self._start_end_episode_distance / max(
